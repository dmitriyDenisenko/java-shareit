...
public class BookingServiceTest {
    ...
    void testGetBookingById() {
        BookingDtoUser bookingDtoFromSQL = bookingService.getBookingById(2L, 1L);
        bookingDtoFromSQL.getItem().setComments(new ArrayList<>());
        assertThat(bookingDtoFromSQL.getId(), equalTo(bookingDtoToUser1.getId()));
    }

    @Test
    void testGetBookingByIdTwo() {
        BookingDtoUser bookingDtoFromSQL = bookingService.getBookingById(1L, 1L);
        bookingDtoFromSQL.getItem().setComments(new ArrayList<>());
        assertThat(bookingDtoFromSQL.getId(), equalTo(bookingDtoToUser1.getId()));
    }
    ...
    void testCreate() {
        ...
        BookingDtoUser bookingDtoFromSQL = bookingService.getBookingById(2L, bookingId);
        bookingDtoFromSQL.getItem().setComments(new ArrayList<>());
        assertThat(bookingDtoFromSQL.getId(), equalTo(bookingDtoToUserCreated.getId()));
    }

    @Test
    void testCreateNotAvailable() {
        assertThrows(ItemNotAvailableException.class, () -> bookingService.create(2L, 3L, createdDto));
    }
    ...
    void testCreateWrongBooker() {
        assertThrows(UserNotExistsException.class, () -> bookingService.create(20L, 4L, createdDto));
    }

    @Test
    void testApproveStatus() {
        BookingDtoUser bookingDtoFromSQL = bookingService.approveStatus(1L, 2L, true);
        bookingDtoFromSQL.getItem().setComments(new ArrayList<>());
        assertThat(bookingDtoFromSQL.getId(), equalTo(bookingDtoToUser2.getId()));
    }

    @Test
    void testApproveStatusRejected() {
        bookingDtoToUser2.setStatus(Status.REJECTED);
        BookingDtoUser bookingDtoFromSQL = bookingService.approveStatus(1L, 2L, false);
        bookingDtoFromSQL.getItem().setComments(new ArrayList<>());
        assertThat(bookingDtoFromSQL.getId(), equalTo(bookingDtoToUser2.getId()));
    ...
    void testApproveStatusNotChangeStatus() {
        assertThrows(BookingNotChangeStatusException.class, () -> bookingService.approveStatus(1L,
                1L, true));
    }

    @Test
    void testApproveStatusNotOwner() {
        assertThrows(UserIsNotOwnerException.class, () -> bookingService.approveStatus(2L,
                2L, true));
    }

    @Test
    void testApproveStatusBookingNotFound() {
        assertThrows(BookingNotFoundException.class, () -> bookingService.approveStatus(1L,
                20L, true));
    }
